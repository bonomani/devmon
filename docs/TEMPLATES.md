...
### Errors
Catching errors and determining their origins is essential. Errors generated by thresholds are indeed just alarms, but we can also encounter real errors.
Errors are propagated from source oids to targets oids. If the error flag set true, the threshold processing bypass and the error, the color and the message are just copyed to be propagated.
#### Connectivity error
...
#### Computational error: 
Occurs when the result is impossible, the value is set to `NaN`
...
Errors are propagated from source oids to targets oids. If the error flag set true, the transforms will bypass the threshold processing and just copy the error, the color and the message.
