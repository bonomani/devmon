...
Notes: 
- In `Devmon` , the term `OID` is abused as used to refer to any variable like the `target OID`, the `numeric OID`, the `index`: This is conveniant, but confusing as this is not the SNMP definition.   
...
### SET transform
The SET transform generates a `repeater` OID with preset values. Each value's 
...
a number or a character string excluding `,{}`. Leading and trailing spaces are ignored.
