...
### Error
Catch errors and determine its origins. Errors generated by thresholds are indeed just alarms, but we can encounter real one.
...
#### Computational error: 
Occurs when the result is impossible -> NaN
...
- Default thresholds like 'clear' or 'blue' can usually be overriden

Errors are propagated from source oids to target oid. If the error flag set true, the transforms will bypass the threshold processing and just copy the error, the color and the message.
