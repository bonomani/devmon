...
$SNMP_Session2::default_avoid_negative_request_ids = 0;
$SNMP_Session2::default_use_16bit_request_ids = 0;
$SNMP_Session2::recycle_socket = 0;
...
sub encode_snmp_request {
    ...
    return $this->wrap_snmp_request(encode_tagged_sequence($reqtype,
...
sub encode_snmp_get_request {
    my ($this, @oids) = @_;
    return encode_snmp_request($this, GET_REQUEST, \@oids);
}

sub encode_snmp_getnext_request {
    my ($this, @oids) = @_;
    return encode_snmp_request($this, GETNEXT_REQUEST, \@oids);
}

sub encode_snmp_getbulk_request {
    my ($this, $non_repeaters, $max_repetitions, @oids) = @_;
    return encode_snmp_request($this, GETBULK_REQUEST, \@oids, $non_repeaters, $max_repetitions);
}

sub decode_snmp_get_response {
    my($this, $response) = @_;
    my @rest;
    @{$this->{'unwrapped'}};
}

sub wait_for_snmp_response {
    my($this, $timeout) = @_;
    $timeout ||= 10.0;
    my($rin,$win,$ein) = ('','','');
...
sub send_get_request_and_receive_response {
    my($this, @oids) = @_;
    return $this->send_request_and_receive_response_with_retries ($this->encode_snmp_get_request (@oids), GET_RESPONSE, \@oids, 1);
}

sub send_getnext_request_and_receive_response {
    my($this,@oids) = @_;
    return $this->send_request_and_receive_response_with_retries ($this->encode_snmp_getnext_request (@oids), GET_RESPONSE, \@oids, 1);
}

sub send_getbulk_request_and_receive_response {
    my($this,$non_repeaters,$max_repetitions,@oids) = @_;
    return $this->send_request_and_receive_response_with_retries ($this->encode_snmp_getbulk_request ($non_repeaters,$max_repetitions,@oids), GET_RESPONSE, \@oids, 1);
}

sub send_request_and_receive_response_with_retries {
    ...
    wait_for_response:
        ($nfound, $timeleft) = $this->wait_for_snmp_response($timeleft);
        if ($nfound > 0) {
            my($response_length);
            $response_length = $this->receive_snmp_response ($response_tag, $oids, $errorp, 1);
            if ($response_length) {
...
sub open {
    ...
    my $family = $ipv4only ? AF_INET : ($SNMP_Session2::ipv6available ? AF_UNSPEC : AF_INET);
...
# 5. Error Handling
sub handle_snmp_error { return SNMP_Error::error(@_) }
sub return_snmp_error { return SNMP_Error::error_return(@_) }
sub handle_ber_error { return SNMP_Error::ber_error(@_) }
...
$dont_wait_flags = 0;
my %the_socket = ();
$SNMP_Session2::errmsg = '';
#$SNMP_Session2::suppress_warnings = 0;
sub timeout { $_[0]->{timeout} }
sub retries { $_[0]->{retries} }
sub backoff { $_[0]->{backoff} }
sub version { $VERSION; }
sub wrap_snmp_request {
    my($this) = shift;
    my($request) = shift;
    encode_sequence (encode_int ($this->snmp_version),
                     encode_string ($this->{community}),
                     $request)
      || return $this->handle_ber_error ("wrapping up request PDU");
}
    ...
    ($snmpver,$community,$request_id,
     ...
     @rest)
        = decode_by_template ($response, "%{%i%s%*{%i%i%i%{%@",
                              $tag);
    return $this->handle_ber_error ("Error decoding response PDU")
      unless defined $snmpver;
    return $this->handle_snmp_error ("Received SNMP response with unknown snmp-version field $snmpver")
    ...
    if ($this->{error_status} != 0) {
      if ($errorp) {
        ...
        return ($community, $request_id,
                $this->handle_snmp_error ("Received SNMP response with error code\n"
                              ."  error status: $errmsg\n"
...
sub receive_snmp_response {
    ...
    return $this->handle_snmp_error ("receiving response PDU: $!")
        unless defined $remote_addr;
    return $this->handle_snmp_error ("short (".length $this->{'pdu_buffer'}
                                      ." bytes) response PDU")
    ...
    if (defined $this->{'remote_addr'}) {
        if (! $this->addresses_equal ($remote_addr, $this->{'remote_addr'})) {
            if ($this->{'debug'} && !$SNMP_Session2::recycle_socket) {
                carp ("Response came from ".&SNMP_Session2::format_ip_address($remote_addr)
                      .", not ".&SNMP_Session2::format_ip_address($this->{'remote_addr'}))
                        unless $SNMP_Session2::suppress_warnings;
            }
            return 0;
        }
    }
    ...
    my ($response_community, $response_id, @unwrapped)
        = $this->unwrap_snmp_response($response, $response_tag, $oids, $errorp);

    if ($response_community ne $this->{community} || $response_id ne $this->{request_id}) {
        if ($this->{'debug'}) {
            carp ("$response_community != $this->{community}")
                unless $SNMP_Session2::suppress_warnings || $response_community eq $this->{community};
            carp ("$response_id != $this->{request_id}")
                unless $SNMP_Session2::suppress_warnings || $response_id == $this->{request_id};
        }
        return 0;
    }
...
sub describe_snmp_session {
    my ($this) = shift;
    print $this->snmp_session_to_string(), "\n";
}

sub snmp_session_to_string {
    my ($this) = shift;
    my ($class, $prefix);
    $class = ref($this);
    ...
    return $class
           .(defined $this->{remote_hostname}
             ...
             ." ".&SNMP_Session2::format_ip_address($this->remote_addr).")"
...
sub close {
    ...
    if (! exists $the_socket{$this->{sockfamily}}
        ...
        close ($this->sock) || $this->handle_snmp_error ("close: $!");
    }
}
...
use warnings;
use Data::Dumper;
use parent 'SNMP_Session2';  # Inherit from SNMP_Session2
...
sub open {
    SNMP_Session2::version(0);
    my $session = SNMP_Session2::open(@_);  # Call the open method from SNMP_Session2
